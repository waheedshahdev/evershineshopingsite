...

{
...

    {
        ...
        $data = array(
                    ...
                    'total_members' => get_total('id', 'tbl_member'),

                    'total_customers' => get_total('id', 'tbl_customers'),

                    'total_orders' => get_total('id', 'tbl_order', 'order_status = "Pending"'),

                    // 'company_profit' => get_query_data('SELECT p_info_id,product_quantity,company_profit, SUM(product_quantity * company_profit) as  c_profit FROM `tbl_order` as O JOIN tbl_order_detail as D on D.order_id = O.id JOIN tbl_product_info as I on I.id = D.p_info_id where O.order_status = "Completed"'),

                    // 'total_cv' => get_query_data('SELECT p_info_id,product_quantity,cv_cost, SUM(product_quantity * cv_cost) as total_cv FROM `tbl_order` as O JOIN tbl_order_detail as D on D.order_id = O.id JOIN tbl_product_info as I on I.id = D.p_info_id where O.order_status = "Completed"'),

                    // 'investment' => get_query_data('SELECT p_info_id,product_quantity,company_total_cost, SUM(product_quantity * company_total_cost) as  c_total_cost FROM `tbl_order` as O JOIN tbl_order_detail as D on D.order_id = O.id JOIN tbl_product_info as I on I.id = D.p_info_id where O.order_status = "Completed"'),
  ...
  if($this->session->userdata('user_type') == 'Admin' )

    {
        ...
        if($submit == 'Submit'){
            ...
            $ref_code = $this->input->post('referal_person_code');
...
elseif ($member_type == 'Referee is in my downline') {
...

{
...

    {
        ...
        $this->load->model("mdl_admin");  
           ...
           $fetch_data = $this->mdl_admin->member_make_datatables();  
           ...
           {  
                ...
                if($row->status == 'Active'){
                    ...
                    $butt = '<a href="#" id = "'.$row->id.'" name="block_member" class="btn btn-danger btn-xs block_member">Block</a>';

                }

                else if($status == 'Block'){

                    $butt = '<a href="#" id = "'.$row->id.'" name="unblock_member" class="btn btn-info btn-xs unblock_member">UnBlock</a>';
                ...
                $sub_array[] = $row->member_email;     

                $sub_array[] = $row->pcv;     

                $sub_array[] = $row->tcv;     

                $sub_array[] = $row->referal_person_code;      

                $sub_array[] = $row->status;     

                $sub_array[] = $row->created_at;  

                $sub_array[] = ''.$butt.' '.'<a href="'.base_url().'admin/member_detail/'.$row->member_code.'" class="btn btn-info btn-xs">Detail</a>';   
           ...
           $output = array(  
                ...
                "recordsTotal"          =>      $this->mdl_admin->member_get_all_data(),  

                "recordsFiltered"     =>     $this->mdl_admin->member_get_filtered_data(),  
...

    {
...

        {
            ...
            if($order_status == 'Completed'){
                    ...
                    // update_data_by_where('tbl_member', $update_data,'member_code='.$member_code.'');

                execute_query('UPDATE tbl_member set pcv = pcv + '.$cv.' where member_code = '.$member_code.'');



                // $select_direct_members = get_query_data('SELECT member_id FROM tbl_tcv_member where direct_referal='.$customer_id.'');

                $select_direct_members = select_columns('direct_referal','tbl_tcv_member', 'member_id='.$member_code.'');

                // print_r($select_direct_members);

                // exit();

                $d_id = $select_direct_members[0]->direct_referal;
                ...
                if(!empty($d_id)){
                    ...
                    // foreach ($select_direct_members as $direct) {

                        execute_query('UPDATE tbl_member set tcv = tcv + '.$tcv.' where member_code = '.$d_id.'');

                        // update_data_by_where('tbl_member', $direct_tcv, 'member_code = '.$direct->member_code.'');

                    // }

                }

                // First Indirect TCV Updation



                 $select_first_indirect = select_columns('first_indirect','tbl_tcv_member', 'member_id='.$member_code.'');

                 $f_id = $select_first_indirect[0]->first_indirect;

                if(!empty($f_id)){

                    $tcv = $cv*25/100;

                    // foreach ($select_first_indirect as $first) {

                        execute_query('UPDATE tbl_member set tcv = tcv + '.$tcv.' where member_code = '.$f_id.'');
                ...
                //Second Indirect TCV Updation

                 $select_second = select_columns('second_indirect','tbl_tcv_member', 'member_id='.$member_code.'');

                 $s_id = $select_second[0]->second_indirect;

                if(!empty($s_id)){

                    $tcv = $cv*12.5/100;

                    // foreach ($select_second as $second) {

                        execute_query('UPDATE tbl_member set tcv = tcv + '.$tcv.' where member_code = '.$s_id.'');
                ...
                // end Second indirect 
                 ...
                 //third Indirect TCV Updation

                 $select_third = select_columns('third_indirect','tbl_tcv_member', 'member_id='.$member_code.'');

                 $t_id = $select_third[0]->third_indirect;

                if(!empty($t_id)){

                    $tcv = $cv*12.5/100;

                    // foreach ($select_third as $third) {

                        execute_query('UPDATE tbl_member set tcv = tcv + '.$tcv.' where member_code = '.$t_id.'');
        ...
        if($submit == 'Update Product Info'){
            ...
            $p_info_id = $this->input->post('p_info_id');
                    ...
                    $data = array(
                                ...
                                'member_retail_price_new' => $this->input->post('member_retail_price_new'),
                                ...
                                'company_profit' => $this->input->post('company_profit'),
...

{
    ...
    $result = delete_data('tbl_products', $id);
...

{
...

    {
        ...
        $this->load->model("mdl_admin");  
           ...
           $fetch_data = $this->mdl_admin->order_make_datatables_member();  
           ...
           $output = array(  
                ...
                "recordsTotal"          =>      $this->mdl_admin->order_get_all_data_member(),  

                "recordsFiltered"     =>     $this->mdl_admin->order_get_filtered_data_member(),  
...
{  
    ...
    $data['member_order_info'] = get_query_data('SELECT * FROM tbl_order JOIN tbl_member on tbl_member.member_code = tbl_order.customer_id where tbl_order.id = '.$id.'');
...

{
...

    {
        ...
        $this->load->model("mdl_admin");  
           ...
           $fetch_data = $this->mdl_admin->withdrawal_make_datatables();  
           ...
           {  
                ...
                $sub_array[] = $row->withdrawal_mode; 

                $sub_array[] = $row->withdrawal_source; 

                $sub_array[] = $row->account_holder; 

                $sub_array[] = $row->badge_name; 

                $sub_array[] = $row->withdraw_amount; 

                $sub_array[] = $row->status; 

                $sub_array[] = $row->created_at;  

                $sub_array[] = '<a href="'.base_url().'admin/withdrawal_detail/'.$row->w_id.'" class="btn btn-info btn-xs">View</a>';   
           ...
           $output = array(  
                ...
                "recordsTotal"          =>      $this->mdl_admin->withdrawal_get_all_data(),  

                "recordsFiltered"     =>     $this->mdl_admin->withdrawal_get_filtered_data(),  
...
public function withdrawal_detail($w_id)
...
{  
...

    {
        ...
        $submit_request = $this->input->post('submit_request');

        if($submit_request == 'Submit Request')

        {

            $request_status = $this->input->post('withdrawal_status');

            $withdrawal_mode = $this->input->post('withdrawal_mode');

            $data = array(

                        'withdraw_date'  => $this->input->post('withdrawal_date'),
            ...
            $this->image_config('bank_receipts');
                ...
                if(!$this->upload->do_upload('bank_receipts'))

                {
                    ...
                    redirect('admin/withdrawal_detail/'.$w_id.'');
...

                {
                    ...
                    $res = update_data('tbl_withdrawl_request',$data,$w_id);
...

                    {
                        ...
                        // $where_2 = 'where id = '.$earning_id.'';

                        $w_amount = $this->input->post('w_amount');

                        $remain_amount = $this->input->post('remain_amount');

                        $amount_remain = $remain_amount - $w_amount;

                        if($request_status == 'Partial Paid'){

                        if($withdrawal_mode == 'Referral Point Bonus (RPB)')

                        {

                            $earning_data = array('referal_amount' => 0, 'cancelation_charges' => 0, 'withdraw_amount' => $amount_remain, 'status' => 'Partial Paid');

                        }

                        elseif ($withdrawal_mode == 'Incentive') {

                            $earning_data = array('incentive_amount' => 0, 'cancelation_charges' => 0, 'withdraw_amount' => $amount_remain,'status' => 'Partial Paid');

                        }

                        elseif ($withdrawal_mode == 'RPB + Incentive Both') {

                           $earning_data = array('incentive_amount' => 0, 'cancelation_charges' => 0, 'withdraw_amount' => 0, 'referal_amount' => 0, 'status' => 'Completed');

                        }

                        update_data('tbl_referal_earnings', $earning_data, $earning_id);

                        }

                        elseif ($request_status == 'Completed') {

                             $earning_data = array('incentive_amount' => 0, 'cancelation_charges' => 0, 'withdraw_amount' => 0, 'referal_amount' => 0, 'withdraw_status' => 'Completed', 'status' => 'Completed');

                             update_data('tbl_referal_earnings', $earning_data, $earning_id);



                             $earning_data = array('status' => 'Completed');

                             update_data_by_where('tbl_withdrawl_request', $earning_data, 'earning_id = '.$earning_id.'');
...

                    {
                        ...
                        $this->session->set_flashdata('error_msg', 'Sorry! Request has Not been sent Due to Error.');
    ...
    $data['withdaw_data'] = get_query_data('SELECT *,W.id as w_id, W.status as w_status, W.withdraw_amount as w_amount, tbl_referal_earnings.withdraw_amount as remain_amount  FROM tbl_withdrawl_request as W

                                                    JOIN tbl_member on tbl_member.member_code = W.member_id

                                                    JOIN tbl_referal_earnings on tbl_referal_earnings.id = W.earning_id where W.id = '.$w_id.'');

    $data['title'] = "Withdrawal Requests";

    $data['view_module'] = "admin";

    $data['view_files'] = "withdrawal_detail";
